...
public sealed class NetworkDevice : INotifyPropertyChanged
{
    ...
    public string DownloadRateText => FormatRate(DownloadRateMbps, _rateUnit);
    ...
    public string ControlDownloadText { get; private set; } = "—";
    public string ControlUploadText { get; private set; } = "—";
    public string ControlBlockedText { get; private set; } = string.Empty;

    public void SetControlIndicators(DeviceControlRule? rule)
    {
        ControlDownloadText = rule?.DownloadLimitMbps is { } download ? $"{download} Mbps" : "—";
        ControlUploadText = rule?.UploadLimitMbps is { } upload ? $"{upload} Mbps" : "—";
        ControlBlockedText = rule?.BlockInternet == true ? "Blocked" : string.Empty;
        OnPropertyChanged(nameof(ControlDownloadText));
        OnPropertyChanged(nameof(ControlUploadText));
        OnPropertyChanged(nameof(ControlBlockedText));
    }
    ...
    {
        get => _isBulkSelected;
        set => SetField(ref _isBulkSelected, value);
    }
