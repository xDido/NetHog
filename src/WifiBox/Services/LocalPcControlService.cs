...
public sealed class LocalPcControlService
{
    ...
    public Task RemoveRuleAsync(CancellationToken cancellationToken = default)
    {
        ...
        return RunPowerShellAsync(
            $"Remove-NetQosPolicy -Name '{Escape(QosPolicyName)}' -Confirm:$false -ErrorAction SilentlyContinue; " +
            $"Remove-NetFirewallRule -DisplayName '{Escape(FirewallRuleName)}' -ErrorAction SilentlyContinue",
            cancellationToken);
    }
