...
public sealed class ArpEnforcementService : IEnforcementService
{
    ...
    private async Task MaintainSessionAsync(NetworkSnapshot network, LibPcapLiveDevice device, CancellationToken cancellationToken)
    {
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    SessionEndedUnexpectedly?.Invoke("The network adapter or IPv4 address changed. NetHog stopped the control session and attempted to restore the affected devices.");
        ...
        catch (Exception) when (!cancellationToken.IsCancellationRequested)
        {
            ...
            SessionEndedUnexpectedly?.Invoke("Npcap stopped responding. NetHog stopped the control session and attempted to restore the affected devices.");
            _ = StopSessionAsync(CancellationToken.None);
        }
    }
    ...
    private void HandleFlowFailure(Exception exception)
    {
        ...
        SessionEndedUnexpectedly?.Invoke($"Npcap could not forward a device packet ({exception.Message}). NetHog stopped the session and attempted to restore ARP entries.");
        _ = StopSessionAsync(CancellationToken.None);
    }
