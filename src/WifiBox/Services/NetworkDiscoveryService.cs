...
{
    public async Task<NetworkSnapshot> ScanAsync(CancellationToken cancellationToken = default)
    {
        ...
        var adapter = FindActiveNetworkAdapter();
    ...
    private static NetworkInterface FindActiveNetworkAdapter()
    {
        ...
        var activeRouteAdapter = defaultRouteIndex is null
            ? null
            : candidates.FirstOrDefault(candidate => candidate.GetIPProperties().GetIPv4Properties()?.Index == defaultRouteIndex);
        var adapter = activeRouteAdapter ?? candidates
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var host = await Dns.GetHostEntryAsync(ipAddress)
                .WaitAsync(TimeSpan.FromMilliseconds(450), cancellationToken);
